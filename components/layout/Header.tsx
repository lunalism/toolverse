...
import { MoonIcon, ChevronDown, ChevronRight } from 'lucide-react'
        ...
        <header className="fixed top-0 left-0 w-full z-50 bg-white border-b border-gray-100 shadow-sm text-gray-800">
            <div className="max-w-screen-xl mx-auto px-6 py-5 flex items-center justify-between">
                    ...
                    {MENU.map(({ label, items, badge }) => {
                        ...
                        return (
                            <div key={label} className="relative">
                                {/* 대분류 메뉴 */}
                                <div 
                                className="flex items-center space-x-1 hover:text-black transition cursor-pointer py-2"
                                ...
                                {isOpen && (
                                    <div className="absolute top-full left-0 pt-2">
                                        {/* 삼각형 화살표 */}
                                        <div className="absolute top-2 left-4 w-3 h-3 bg-white rotate-45 border-l border-t border-gray-200 z-20"></div>
                                        
                                        {/* 메뉴 콘텐츠 */}
                                        <div
                                            className={`bg-white shadow-xl border border-gray-200 rounded-md min-w-[180px] z-10 overflow-hidden mt-2
                                        ...
                                        >
                                            {items.map(({ label: itemLabel, href }) => (
                                                ...
                                                key={itemLabel}
                                                href={href}
                                                className="block px-4 py-3 hover:bg-gray-50 text-sm whitespace-nowrap transition-colors"
                ...
                <div className="flex items-center space-x-4 text-gray-500 text-sm">
                    <MoonIcon className="w-5 h-5 cursor-pointer hover:text-gray-700 transition" />
