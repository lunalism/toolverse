// src/app/pdf/reorder/page.tsx (최종 수정)
...
export default function PdfReorderPage() {
  ...
  const handleCreatePdf = async () => { /* ... 이전과 동일 ... */ if (!file || pages.length === 0) return; setIsLoading(true); const formData = new FormData(); formData.append('file', file); const pagesOrder = pages.map(p => p.originalIndex); formData.append('pagesOrder', JSON.stringify(pagesOrder)); try { const response = await fetch('/api/pdf/reorder', { method: 'POST', body: formData, }); if (!response.ok) throw new Error('서버에서 PDF 생성에 실패했습니다.'); const blob = await response.blob(); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `toolverse-reordered_${new Date().getTime()}.pdf`; document.body.appendChild(a); a.click(); a.remove(); window.URL.revokeObjectURL(url); } catch (error) { console.error(error); alert('PDF를 생성하는 중 오류가 발생했습니다.'); } finally { setIsLoading(false); } };

  // 👇 페이지 상태를 초기화하는 함수를 추가합니다.
  const handleReset = () => {
    setFile(null);
    setPages([]);
  };
      ...
      {pages.length > 0 && !isLoading && (
        <div>
          ...
          <div className="mt-8 flex justify-center items-center gap-4">
            <Button onClick={handleReset} className="px-8 py-4 bg-white text-gray-700 font-bold border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 transition-colors">
              다시 선택
            </Button>
             <Button onClick={handleCreatePdf} className="px-8 py-4 bg-gradient-to-r from-teal-400 to-blue-600 text-white font-bold rounded-lg shadow-md hover:opacity-90 disabled:bg-gray-400 disabled:from-gray-400 disabled:to-gray-400 disabled:cursor-not-allowed">
                새로운 PDF로 만들기
             </Button>
          </div>
        </div>
